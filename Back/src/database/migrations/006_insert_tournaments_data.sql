...
SELECT 'John Doe', 'johndoe', 'john@example.com', 'hashedpassword4', 'Spider-Man-Avatar.png', 1, datetime('now')
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'john@example.com');

INSERT INTO users (name, username, email, password_hash, avatar_url, is_online, last_seen_at)
SELECT 'Maria Silva', 'maria', 'maria@example.com', 'hashedpassword5', 'Moana-Avatar.png', 0, datetime('now', '-30 minutes')
