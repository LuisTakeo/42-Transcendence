...
const chatUsers: Map<string, any> = new Map();
...
function handleWebSocketConn(connection: WebSocket, 
    ...
    connection.on('message', (message: string) => {
        ...
        try {
            ...
            switch (data.type) {
                case 'join_chat':
                    chatUsers.set(userId, connection);
                    break;
                
                case 'chat_message':
                    handleChatMessage(userId, data.message);
                    break;
...
export { handleWebSocketConn, chatUsers, GameRoom };
