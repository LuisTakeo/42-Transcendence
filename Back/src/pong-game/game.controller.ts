...
function createGameRoom(userId: string, connection: any): number {
    const roomId = userId + '-' + Date.now(); // Gera um ID único baseado no userId e timestamp
...
function joinGameRoom(userId: string, connection: any, roomId: string) {
    // verificar se há salas disponíveis, se não houver,
    const roomAvailable = findAvailableRoom();
    if (gameRooms.size === 0 || !roomAvailable) {
        console.log(`Nenhuma sala disponível para o usuário ${userId}`);
        return;
    }
    // #TODO: CONTINUAR DAQUI
    // roomAvailable.players.set(userId, connection);
    // userToRoom.set(userId, roomAvailable.id);
